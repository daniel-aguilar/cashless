...
public interface TransactionRepository extends JpaRepository<Transaction, Integer> {
  ...
  @Query("FROM Transaction WHERE sender = ?1 OR recipient = ?1 ORDER BY date DESC")
  List<Transaction> findLatestTransactions(Account account, Pageable page);

  @Query("FROM Transaction WHERE sender.game = ?1 AND recipient.game = ?1 ORDER BY date DESC")
